...
.ほげの図
image::yhpg-last-52.svg[width=100, pdfwidth=50mm]

「 `radialGradient` 」というタグがサポートされないらしい。

とはいえ

[quote, https://developer.mozilla.org/ja/docs/Web/SVG/Element/radialGradient]
...
asciidoctor-mathematical を入れたらレンダリングできた。
...
\begin{align*}
  ...
  G(x) &= \frac{1+\frac{x}{a}}{1+\frac{1}{1+\frac{1}{e^x}}} \\
  R(x) &= \sum_{k=0}^{\infinity} \sin^2(x^k) \\
  Q_i(x,y) &= \frac{ {\displaystyle \sum_{k=0}^{i} \sin^2(1+x^k) } }{ {\displaystyle \sum_{k=0}^{i} \cos^2(1+y^k) }  }
\end{align*}
++++

本文中にドルで挟む $\sum_{k=1} \frac{1}{k}$ のは、無理。

行頭に「latexmath」と書いてみると
